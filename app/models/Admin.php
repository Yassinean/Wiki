...
class adminModel
{
    ...
    {
        $sql = db::connect()->prepare("SELECT * from wikis join users on users.id = wikis.auteur_id");
        $sql->execute();
        $wikis = $sql->fetchAll();
        return $wikis;
    }
    ...
    {
        $sql = db::connect()->prepare("UPDATE wikis set archived = 'archived' where idWiki = :id_wiki ");
    ...
    {
        $sql = db::connect()->prepare("UPDATE wikis set archived = 'notArchived' where idWiki = :id_wiki ");
        $sql->bindParam(':id_wiki', $id_wiki);
        return $sql->execute();
    }
}
